...
MODEL_CLASSES = {"gpt2": (GPT2Config, MultiHopGen, GPT2Tokenizer)}
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                if not any(gn in n for gn in gpt2_params)
            ]
        }
    ]
    optimizer = AdamW(
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    logger.info(
                        "Step: {} | Loss: {:.4f} | Gen loss: {:.4f} | Gate Loss: {:.4f} | Triple Loss: {:.4f}".format(
                            global_step,
...
def generate(args, generator, model, dataset):
    ...
    loader = DataLoader(
        dataset, sampler=sampler, batch_size=args.per_gpu_eval_batch_size
    )
    total_hypos = []
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        default="ppl",
        type=str,
        help="choose between ppl and bleu",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Discount factor in multi-hop reasoning",
    )
    parser.add_argument(
        "--alpha", type=float, default=3.0, help="loss = gen_loss + alpha * gate_loss"
    )
    parser.add_argument(
        "--beta", type=float, default=5.0, help="coefficient of weak supervision"
    )
    parser.add_argument(
        "--aggregate_method",
        type=str,
    ...
    parser.add_argument(
        "--warmup_ratio", default=0, type=float, help="Linear warmup over warmup_ratio."
    )
    ...
    tokenizer = tokenizer_class.from_pretrained(
        args.model_name_or_path, do_lower_case=False
    )
    ...
    if args.do_eval:
        ...
        logger.info(
            "Test evaluate {}: {:.4f}".format(
                args.evaluate_metrics, result[args.evaluate_metrics]
            )
        )
