    ...
    def generate(
        ...
        **kwargs
        ...
        for i, (k, v) in enumerate(sample.items()):
            assert (
                k_order[i] == k
            ), "generator input sample should be Dict with keys ordered in {}!".format(
                k_order
            )
        ...
        # initialize buffers
        scores = (
            src_tokens.new(bsz * beam_size, max_len + 1)
        ...
        tokens = (
            src_tokens.new(bsz * beam_size, max_len + 2)
            .to(src_tokens.device)
            .long()
            .fill_(self.pad)
        )
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                if batch_idxs is not None:
                    ...
                    corr = batch_idxs.to(src_tokens.device) - torch.arange(
                        batch_idxs.numel()
                    ).type_as(batch_idxs).to(src_tokens.device)
                    reorder_state.view(-1, beam_size).add_(
                        corr.unsqueeze(-1) * beam_size
                    )
                encoder_outs = reorder_encoder_out(encoder_outs, reorder_state)

            encoder_outs["input_ids"] = torch.cat(
                (encoder_outs["input_ids"][:, :src_len], tokens[:, : step + 1]), dim=1
            )
            encoder_outs["attention_mask"] = torch.cat(
                (
                    encoder_outs["attention_mask"],
                    encoder_outs["attention_mask"].new_ones(
                        encoder_outs["attention_mask"].size(0), 1
                    ),
                ),
                dim=1,
            )
            encoder_outs["position_ids"] = torch.cat(
                (
                    encoder_outs["position_ids"],
                    encoder_outs["position_ids"].new_ones(
                        encoder_outs["position_ids"].size(0), 1
                    )
                    * step,
                ),
                dim=1,
            ...
            probs, out_gate, _ = model_forward(
                input_ids=encoder_outs["input_ids"],
